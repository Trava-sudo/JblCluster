...
public class JblAlarmNotPeripheralNode extends JpsEvtAlarm implements JblVisitableEvent {

    @Override
    public Long messageId() {
        return JmsMessageIDBundle.ALARM_JBL_VIRTUAL_PERIPHERAL_NONE_PERIPHERAL_NODE;
    }
    ...
    public CompletableFuture<AsyncResult<JmsCommonEvent>> accept(JpsEventMapperVisitor visitor) {
        ...
        try {
            ((JblEventMapperVisitor) visitor).map(this).thenAccept(jmsEventAsyncResult -> {
                ...
                promise.complete(new AsyncResultBuilder<JmsCommonEvent>().withSuccess().withResult(jmsEventAsyncResult.result()).build());
